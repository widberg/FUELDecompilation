...
{
    EnterCriticalSection(&dmalloc_mutex);
}
...
{
    LeaveCriticalSection(&dmalloc_mutex);
}
#endif
...
static	void	process_environ(const char *option_str)
{
  ...
  previous_logpath = dmalloc_logpath;
...
#endif /* SIGNAL_OKAY */
InitializeCriticalSection(&dmalloc_mutex);
