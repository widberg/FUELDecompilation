			       ...
			       const char *reason, const char *where)
{
  ...
  if (dmalloc_errno == DMALLOC_ERROR_UNDER_FENCE) {
    ...
    if ((unsigned int)dump_size > user_size + FENCE_OVERHEAD_SIZE) {
    ...
    dump_size = DUMP_SPACE;
    offset = 0;
    if (user_size > 0 && (unsigned int)dump_size > user_size) {
      dump_size = user_size;
    }
  }
...
void	_dmalloc_chunk_log_stats(void)
{
  ...
  diff = (PNT_ARITH_TYPE)_dmalloc_heap_high -
